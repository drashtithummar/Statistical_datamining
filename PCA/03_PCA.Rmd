...
> `USArrests` classic R data.frame
> `df` tibble (tidyverse) data.frame
...
# can be handy to manually feed back to R codeS
dput(df$State)
```
...
cat("Mean:\n")
apply(df[-1], 2, mean)
df %>% select(-State) %>% apply(2,mean)

cat("Var:\n")
df %>% select(-State) %>% apply(2,var)

cat("STDEV:\n")
df %>% select(-State) %>% apply(2,sd)
...
```{r}
ggbiplot(pca_fit, scale = 0,labels = pca_fit$x %>% rownames())
```
